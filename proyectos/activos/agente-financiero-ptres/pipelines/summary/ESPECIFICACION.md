...
| Fuente sin columna de moneda (ej. Engineering) | **Moneda MXN — confirmado por el cliente.** Todas las provisiones de Engineering son en pesos mexicanos. |
...
| Discrepancia entre provisión fuente y Summary anterior | Alertar solo si la variación supera **20% Y el monto absoluto es mayor a $50,000 MXN** — confirmado por el cliente. Variaciones menores (proyectos que arrancan/terminan) son ruido normal. |
...
7. ~~**Confirmar con el cliente la moneda de las provisiones de Engineering**~~ — **resuelto (2026-06-27): todas las provisiones de Engineering son MXN**, confirmado por el cliente. El agente asigna MXN directamente sin reportar alerta ni pedir confirmación.
8. ~~**Provisiones "reabiertas":**~~ — **resuelto (2026-06-27): el agente debe buscar historial en hojas anteriores.** Si un proyecto aparece como `PROVISION` en una fuente pero ya existió en meses anteriores del Summary (con cualquier estatus), el agente lo identifica como **reapertura** y lo indica explícitamente en el resumen del Paso 7 (ej. "Provisión reabierta — última aparición: Cancelar en 2026_Feb, monto: $X"). El usuario puede aprobarlo o rechazarlo con contexto completo.
9. 🔴 **[CRÍTICO — DESPLIEGUE]** `chmod 600 agente.db` al arrancar el servidor por primera vez — sin esto cualquier usuario del sistema puede leer directamente el SQLite (datos financieros de P3, hashes de contraseñas, audit log). Agregar al script de inicio de Docker/Compose cuando se cree esa configuración.
10. ~~**Criterio de alerta por discrepancia de monto:**~~ — **resuelto (2026-06-27): alertar si variación > 20% Y monto > $50,000 MXN**, confirmado por el cliente. Actualizado en sección 9.
